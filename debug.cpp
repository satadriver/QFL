	...
	do
	{
		...
		{	
			if ( (pid == node->pid)  && (node->bid.base <= dataAddr && node->bid.top > dataAddr) ) 
			{
				...
				//ret = ProcMemProtect(hp, node->bid.base, (SIZE_T)size,PAGE_EXECUTE_READWRITE);
				...
				{
					if (codeAddr >= m_moduleBase && codeAddr < m_moduleBase + m_moduleSize) {
						if (node->startTime == 0) {
				...
				//printf("access data address:%p,code address:%p\r\n",dataAddr,codeAddr);
				int tmp = 0;
			}
...
#ifdef _WIN64
					ret = GetCallChain(hp, (LPVOID*)context.Ebp);
#else
					ret = GetCallChain(hp,(LPVOID*)context.Ebp);
#endif
...
int Debug::SetReceiveBP() {
	...
	ret = m_breakPoint->SetProcBreakPoint(m_pid, m_recv, HOOK_NETWORK_FUN, 0);
	ret = m_breakPoint->SetProcBreakPoint(m_pid, m_recvfrom, HOOK_NETWORK_FUN, 0);
	ret = m_breakPoint->SetProcBreakPoint(m_pid, m_WSARecv, HOOK_NETWORK_FUN, 0);
	ret = m_breakPoint->SetProcBreakPoint(m_pid, m_WSARecvFrom, HOOK_NETWORK_FUN, 0);
	...
	if (m_moduleBase == 0 && m_moduleSize == 0) {
		string fn = RemovePath(m_filename);
		HMODULE hm = GetProcModule(m_pid, fn.c_str());
		if (hm) {
			m_moduleBase = (char*)hm;

			char data[0x1000];
			SIZE_T cnt = 0;
			HANDLE hp = OpenProcess(PROCESS_ALL_ACCESS, 0, m_pid);
			if (hp) {
				int ret = ReadProcessMemory(hp, m_moduleBase, data, 0x1000, &cnt);
				
				PIMAGE_DOS_HEADER pDos = (PIMAGE_DOS_HEADER)data;
				PIMAGE_NT_HEADERS pNt = (PIMAGE_NT_HEADERS)((char*)data + pDos->e_lfanew);
				m_moduleSize = pNt->OptionalHeader.SizeOfImage;
				CloseHandle(hp);
			}
