...
int inlinehook64(BYTE* newfun, BYTE* hookaddr, PROC* keepaddr, const WCHAR* funcname) {
	...
	if (hookaddr == newfun)
	{
		log(L"[liujinguang]inlinehook64 function:%ws already exist\r\n", funcname);
		return FALSE;
	}
	...
	if (trump <= 0)
	{
		log(L"[liujinguang]inlinehook64 addTrump function:%ws error\r\n", funcname);
		return FALSE;
	}
	...
	while (codelen < minimum)
	{
		...
		if (instructionlen <= 0)
		{
			log(L"[liujinguang]inlinehook64 function:%ws address:%p hde64_disasm error\r\n", funcname, oldfun);
		...
		if (total == 0)
		{
			...
			else if (*oldfun == 0xeb)
			{
				...
				{
					log(L"[liujinguang]inlinehook64 function:%ws jump to target address:%p out of range on base:%p\r\n", funcname, oldfun, oldcode);
					deleteTrump(funcname);
					return FALSE;
				}
				memcpy(trump->code + codelen, oldfun, instructionlen);
			...
			{
				log(L"[liujinguang]inlinehook64 function:%ws address:%p found irregular jump instruction: %02x %02x\r\n", funcname, oldfun, *oldfun, *(oldfun + 1));
		...
		if (total >= 1024)
		{
			log(L"[liujinguang]inlinehook64 function:%ws something error!\r\n", funcname);
			//__debugbreak();
			return FALSE;
		}
	...
	if (result == 0)
	{
		log(L"[liujinguang]inlinehook64 VirtualProtect function:%ws address:%p error\r\n", funcname, oldcode);
		deleteTrump(funcname);
		return FALSE;
	}
	...
	*keepaddr = (FARPROC) & (trump->code);
	...
	log(L"[liujinguang]inlinehook64 function:%ws, hook size:%d ,tramp address:%p, instruction address:%p,new func address:%p,keep address:%p",
...
int inlinehook32(BYTE* newfun, BYTE* hookaddr, PROC* keepaddr, const WCHAR* funcname) {
	...
	while (codelen < minimum)
	{
		...
		if (total == 0)
		{
			...
			else if (*oldfun == 0xeb)
			{
				...
				{
					log(L"[liujinguang]inlinehook32 function:%ws jump to target address:%p out of range on base:%p\r\n", funcname, oldfun, oldcode);
					deleteTrump(funcname);
					return FALSE;
				}
			}
			...
			{
				log(L"[liujinguang]inlinehook32 function:%ws address:%p found irregular jump instruction: %02x %02x\r\n", funcname, oldfun, *oldfun, *(oldfun + 1));
	...
	if (result == 0)
	{
		log(L"[liujinguang]inlinehook32 VirtualProtect function:%ws address:%p error\r\n", funcname, oldcode);
		deleteTrump(funcname);
		return FALSE;
	}
	...
	*keepaddr = (FARPROC) & (trump->code);
	...
	log(L"[liujinguang]inlinehook32 function:%ws,hook size:%d,tramp address:%p,instruction address:%p,new func address:%p,keep address:%p",
		funcname, codelen, trump->code, oldcode, newfun, keepaddr);
	WCHAR szout[1024];
	WCHAR* loginfo = szout;
	int len = 0;
